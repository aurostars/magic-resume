  ...
  return {
    async run(args) {
      ...
      const response = stage === "api"
        ? { id: "api-new", faas_url: "https://magic.solutionsuite.cn/api/faas/api-new" }
        : stage === "web"
          ? { id: "web-new", faas_url: "https://magic.solutionsuite.cn/api/faas/web-new" }
          : { id: "vv6BtLE8MTR", html_box_url: "https://magic.solutionsuite.cn/html-box/vv6BtLE8MTR" };
...
function healthyResponse(input: RequestInfo | URL, apiId = "api-new", releaseId = RELEASE_ID): Response {
  const url = String(input);
  if (url.includes(`/api/faas/${apiId}?`)) {
    return Response.json(
      { error: "Not found", code: "notFound" },
      { status: 404, headers: { "X-Magic-Resume-Faas": "magic-resume-api" } },
    );
  }
  const runtime = {
    platform: "miaobi",
    apiFunctionUrl: `https://magic.solutionsuite.cn/api/faas/${apiId}`,
